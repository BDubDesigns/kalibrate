...
* **Jurisdictional Controls (Defense-in-Depth):** Coarse, best-effort IP-based geofencing (including blocks targeting Washington State and Nevada) is used only as a supplemental safeguard and is not relied upon as a sole compliance mechanism, given the limitations of IP geolocation (VPNs, mobile carriers, shared networks, etc.). Live trading access must additionally be governed by account-level jurisdiction checks during onboarding (e.g., user attestations and, where appropriate, KYC-style residency verification) and, when available, provider-side or broker/exchange-level restrictions that prevent API keys or accounts from being used in prohibited jurisdictions.
